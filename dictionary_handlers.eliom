...
let upload_user_avatar_handler myid () ((), (cropping, photo)) =
  let avatar_dir =
    List.fold_left Filename.concat
      (List.hd !Dictionary_config.avatar_dir)
      (List.tl !Dictionary_config.avatar_dir) in
  let%lwt avatar =
...
let%server forgot_password_handler =
  Os_handlers.forgot_password_handler
    Dictionary_services.settings_service
  ...
  | e ->
    ...
    then
      ...
      then
        ...
        in
        Dictionary_base.App.send
          (Dictionary_page.make_page (Os_page.content page))
      else
        let page = [ div
...
let%shared settings_handler myid_o () () =
  let%lwt content = match myid_o with
    | Some _ -> Dictionary_settings.settings_content ()
    | None -> Lwt.return [ p [%i18n log_in_to_see_page ~capitalize:true]]
  in
  Dictionary_container.page myid_o content

let%server update_language_handler () language =
  Os_session.connected_wrapper Dictionary_language.update_language
    (Dictionary_i18n.language_of_string language)

let%client update_language_handler () language =
  Dictionary_i18n.(set_language (language_of_string language));
